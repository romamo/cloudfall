...
This workflow installs a single-node Loki backend, loopback-only Grafana, an
mTLS-only Nginx ingestion gateway, and Grafana Alloy collectors. It never
stops, disables, reconfigures, or removes Filebeat, Metricbeat, Elasticsearch,
Kibana, or Logstash.
...
- Loki and Grafana bound to `127.0.0.1`;
- a gateway that exposes only `/loki/api/v1/push` and requires a validated
  client certificate;
- TSDB schema v13 with bounded filesystem retention; and
- pinned Debian package versions from Grafana's stable APT repository.

The gateway does not expose Loki's query, administration, deletion, readiness,
or metrics APIs. Grafana remains reachable only through the backend host, for
example with an SSH tunnel. This first slice is a proving deployment, not a
...
time. Loki and Alloy configuration are validated with their native binaries;
Nginx is checked with `nginx -t`; services must then pass readiness checks.
